...
PRACTICA #10
INTRUCCIONES:
Barra Espaciadora ----> Activar y Desactivar animacion
...
bool circuitoD = false;
bool circuitoR = false;
bool recorrido1D = true;
bool recorrido2D = false;
bool recorrido3D = false;
bool recorrido4D = false;
bool recorrido5D = false;
bool recorrido6D = false;
bool recorrido1R = true;
bool recorrido2R = false;
bool recorrido3R = false;
bool recorrido4R = false;
bool recorrido5R = false;
bool recorrido6R = false;
...
void animacion()
{
	...
	if(circuitoD)
	{
		...
		if (recorrido2D)//ESTADO E
		{
			movKitZ ++;
			movKitY -= 2.33;
			if (movKitZ >155 )//TRANSICION A
			{
				recorrido2D = false;
				recorrido3D = true;
			}
		...
		if(recorrido3D)//ESTADO A
		{
			rotKit = 90;
			movKitX++;
			
			if(movKitX > 125) //TRANSICION B
			{
				recorrido3D = false;
				recorrido4D = true;
		...
		if(recorrido4D)//ESTADO B
		{
			rotKit = 210;
			movKitZ -=2.48;
			movKitX--;
			if(movKitZ < 155 && movKitX < 0)//TRANSICION D
			{
				recorrido4D = false;
				recorrido5D = true;
			}
		...
		if(recorrido6D)//ESTADO C 
			
		{
			movKitZ++;
			
			
			if(movKitZ > 0 )//TRANSICION E
			{
	...
	if (circuitoR)
	{
		...
		if (recorrido3R)//ESTADO A
		{
			...
			if (movKitX > 105 && movKitZ > 158.08) //TRANSICION B
			{
		...
		if (recorrido4R)//ESTADO B
		{
			rotKit = 270;
			movKitX --;
			if (movKitX < 0)//TRANSICION D
			{
				recorrido4R = false;
				recorrido5R = true;
			}
		...
		if (recorrido5R)//ESTADO C 

		{	
			rotKit = 180;
			movKitZ --;
			movKitY +=2.4;
			if (movKitZ < 135)//TRANSICION E
			{		
				recorrido5R = false;
				recorrido1R = true;
			}
...
void keyboard ( unsigned char key, int x, int y )  // Create Keyboard Function
{
	switch ( key ) {
		...
		case 'i':
		case 'I':
				circuitoD^= true; //Activamos/desactivamos la animacíon
				g_fanimacion = false;
			break;
