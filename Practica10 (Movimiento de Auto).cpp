...
GLfloat m_dif1[] = { 0.0f, 0.2f, 1.0f, 1.0f };
// Diffuse Light Values
...
{
	glClearColor(1.0f, 0.5f, 0.f, 0.0f);
	// Negro de fondo 
...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	pintaTexto(-12, 10.5, -14, (void *)font, "Movimiento de Auto");
	glColor3f(1.0, 1.0, 1.0);
	glEnable(GL_LIGHTING);
	glEnable(GL_TEXTURE_2D);
...
void animacion() {
	...
	if (g_fanimacion && movKit == 0) {
		reversa = false;
	}

	if (g_fanimacion && movKit < 100 && !reversa) { //hacia delante 
		movKit += 0.5;
	}
	if (g_fanimacion && movKit == 100) {
		reversa = true;
	}

	if (g_fanimacion && movKit > 0 && reversa) { //hacia atras
		movKit -= 0.5;
	}
